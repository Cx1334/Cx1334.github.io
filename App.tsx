...
import { 
  ...
  HardDrive, PanelRightClose, PanelRightOpen, WifiOff, CalendarCheck, AlertCircle, Key
} from 'lucide-react';
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar relative">
           {isSettingsOpen && (
              <div className="absolute top-4 right-4 z-50 w-80 bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 p-5 animate-in fade-in zoom-in duration-200">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4"><button onClick={() => handleGistSync('push')} disabled={isSyncing} className="flex flex-col items-center justify-center gap-2 p-3 bg-slate-50 dark:bg-slate-700 hover:bg-slate-100 dark:hover:bg-slate-600 rounded-lg transition-colors border border-slate-200 dark:border-slate-600 disabled:opacity-50"><Upload size={20} className="text-brand-500" /><span className="text-xs font-medium">上传备份</span></button><button onClick={() => handleGistSync('pull')} disabled={isSyncing} className="flex flex-col items-center justify-center gap-2 p-3 bg-slate-50 dark:bg-slate-700 hover:bg-slate-100 dark:hover:bg-slate-600 rounded-lg transition-colors border border-slate-200 dark:border-slate-600 disabled:opacity-50"><Download size={20} className="text-indigo-500" /><span className="text-xs font-medium">拉取恢复</span></button></div>
